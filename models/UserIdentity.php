...
class UserIdentity extends Users implements IdentityInterface
{
    ...
    public function getPremissionsByParentName($name_parent): array
    {
        $auth = Yii::$app->authManager;
        $permissions = $auth->getPermissionsByUser($this->public_key);

        $items = [];
        foreach ($permissions as $key => $val) {

            if ($auth->getChildren($name_parent)[$key] ?? false) {
                $items[] = ['key' => $key, 'name' => $val->description];
            }
        }
    ...
    public function isPremissionByDescription($description): bool
    {
        $auth = Yii::$app->authManager;
        $permissions = $auth->getPermissionsByUser($this->public_key);
    ...
    public function isPremissionByName($name): bool
    {
        $auth = Yii::$app->authManager;
        $permissions = $auth->getPermissionsByUser($this->public_key);
