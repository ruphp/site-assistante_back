...
$roles = Roles::find()->where(['public_key'=> Yii::$app->user->identity->getPublicKey()])->all();
$html = '
<div class="uk-card uk-card-body uk-card-default stat_all">
    <h3 class="uk-card-title">Статистика использования</h3> 
    <div uk-grid class="uk-child-width-1-2 uk-margin">   
        <div>';
if(count($roles)) {
    $html .='<div>';
    $html .='<label class="control-label">Выбор роли</label>';

    $params = [
        'prompt' => ['text' => 'Без учета ролей', 'options' => ['value' => 0, 'class' => 'prompt'/*, 'label' => 'Select'*/]],
        'class'  => 'form-control',
        'id'  => 'chart_usage_list_roles'
    ];
    $html .='<div>';
    $html .= Html::dropDownList('id_role_in_system', null, ArrayHelper::map($roles, 'id_role_in_system', 'name'), $params);
    $html .='</div>';
    $html .='</div>';
}
