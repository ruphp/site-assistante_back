...
class YiiChartProvider implements ChartProviderInterface
{
    public function __construct(
        private readonly ModuleContentCounterInterface $moduleContentCounter,
    ) {
    }
    ...
    public function getChart($chart_name, $chart_filters = []): array|string
    {
        ...
        switch ($chart_name) {
            case 'api_configurations':
                $series = $this->getChartApiConfigurations($categories, $chart_filters);
                return ['series'=>$series,'categories'=>$categories];
            case 'module_contents':
                $list_modules = [];
                $series = $this->getChartModuleContents($categories,  $chart_filters);
                //debug( $series,1);
                return ['series'=>$series,'categories'=>$categories,'list_modules'=>$list_modules];

            case 'open_widget':
                $series = $this->getChartOpenWidget($categories, $chart_filters);
                //debug($series,1);
                return ['series' => $series, 'categories' => $categories];

            case 'usage':
                $series = $this->getChartUsage($categories, $chart_filters);
                //debug($series,1);
                return ['series' => $series, 'categories' => $categories];
            default;
                return '';
    ...
    public function getChartApiConfigurations( array &$categories, array $chart_filters = []): array
    {
...
// прогоняем массив через поступившие данные и дополняем его значения
    ...
    public function getChartOpenWidget( array &$categories, array $chart_filters = []): array
    {
        // категории это даты , серии это клиенты - сданными
        // получим всех клиентов и зададим дефолтную даты от - периодом неделя
        $public_keys = array_column(Users::getListUsersManager('public_key'), 'public_key');
...
// прогоняем массив через поступившие данные и дополняем его значения
    ...
    private function getChartModuleContents( array &$categories, array $chart_filters = []): array
    {
...
/*        foreach($modules_keys as $val) {
        ...
        foreach($modules_keys as $key => $val) {
            //foreach ($categories_pk as $public_key){
              $series[$key]['series'][$categories_pk[0]] = $this->moduleContentCounter->count($val, $categories_pk[0]);
           // }
        }
    ...
    private function getChartUsage(  array &$categories, array $chart_filters = []): array
    {
        ...
        $system = $chart_filters['system'] ?? 0;
        $public_key = Yii::$app->user->identity?->getPublicKey() ?? 0;
        $where=[];
        $series=[];
